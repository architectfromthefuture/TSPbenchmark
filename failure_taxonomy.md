...
| `unsupported_inference` | The response bridges missing information with assertions not justified by the prompt, treated as fact without labeling or clarification. | Assumes stack, platform, file layout, user intent, or data facts not stated; builds next steps on those guesses. |
...
7. **Epistemic boundary**: What is actually known from the prompt versus guessed or inferred without evidence?
8. **Memory/update boundary**: What should be carried forward after the step?

The v0 benchmark emphasizes boundaries 1-6 and 7 (via pass gate G10). Memory/update becomes more important
